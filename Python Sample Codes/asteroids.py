...
class Game(arcade.Window):
    ...
    def update(self, delta_time):
        ...
        for asteroid in self.asteroids:
            ...
            self.asteroid.wrap(SCREEN_WIDTH, SCREEN_HEIGHT)
    ...
    def check_collisions(self):
        ...
        for asteroid in self.asteroids:
            ...
            if self.ship.alive and self.asteroid.alive:
                    too_close = self.ship.radius + self.asteroid.radius

                    if (abs(self.ship.center.x - self.asteroid.center.x) < too_close and
                                abs(self.ship.center.y - self.asteroid.center.y) < too_close):
                        # its a hit!
        ...
        for bullet in self.bullets:
            for asteroid in self.asteroids:
                ...
                # Make sure they are both alive before checking for a collision
                if self.bullet.alive and self.asteroid.alive:
                    too_close = self.bullet.radius + self.asteroid.radius
                    
                    if (abs(self.bullet.center.x - self.asteroid.center.x) < too_close and
                    abs(self.bullet.center.y - self.asteroid.center.y) < too_close):
                        # its a hit!
                        self.bullet.alive = False
                        self.asteroid.alive = False
                        self.score += self.asteroid.hit()[1]
    ...
    def cleanup_zombies(self):
        ...
        """
        for bullet in self.bullets:
            if (abs(self.ship.center.x - self.bullet.center.x) > BULLET_LIFE or
                                abs(self.ship.center.y - self.bullet.center.y) > BULLET_LIFE):
                self.bullet.alive = False
        ...
        for asteroid in self.asteroids:
            if not self.asteroid.alive:
                ...
                self.asteroids += self.asteroid.hit()[0]
    ...
    def on_key_press(self, key: int, modifiers: int):
        ...
        if self.ship.alive:
            ...
            if key == arcade.key.SPACE:
                ...
                bullet = Bullet()
                self.bullet.store(self.ship)
                self.bullet.fire(bullet.angle)
                self.bullet.draw()
